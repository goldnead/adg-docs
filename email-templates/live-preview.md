...
and renders it through the exact same path as a real send:

```
Bard nodes → BardHtmlRenderer → HTML → MergeVariables::apply() → iframe contents
```

returning the merged subject and HTML body.
...
| `'enabled' => false` | The preview route is not registered |
...
collection. Check that the addon booted — a `'enabled' => false` in config, or a boot-order problem in a
custom service provider that touches the collection.

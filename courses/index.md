...
- **Locks.** Sequencing by section or by lesson, prerequisites, phases with a test-out, and
  drip by week from enrollment. A locked lesson carries a `lock_reason`. See
  [Drip and locks](/courses/locks).
- **Lesson types that complete differently.** A video by being watched, a text by being
  acknowledged, a quiz only by the code that graded it. See
...
- **A rollup** per learner and course: status, percent, completed and total, and the lesson
  to continue with.
- **Antlers tags and a form route** for the front end, and one read-only screen in the
  Control Panel, **Course Progress**.

## What it is not

- **Not a player.** It ships no lesson template and no video player. The tags hand a
  template the data; how a lesson looks is the site's.
...
- **Not a quiz engine.** A quiz, an assignment or a reflection completes only when the code
  that checked it calls `Courses::completeLesson()`. Grading is somebody else's.
...
- [Lesson types and proof](/courses/lesson-types)
- [Drip and locks](/courses/locks)
- [Reference](/courses/reference): facade, events, tables, routes, permission
